class Solution {
    ...
    public int numEnclaves(int[][] grid) {
        ...
        for (int i = 0; i < visited.length; i++) {
            ...
            if (board[i][board[0].length - 1] == 1 && !visited[i][board[0].length - 1]) {
        ...
        for (int i = 0; i < visited[0].length; i++) {
            ...
            if (board[board.length - 1][i] == 1 && !visited[board.length - 1][i]) {
    ...
    private void dfs(int i, int j, int[][] board, boolean[][] visited, int[] deltaRow, int[] deltaCol) {
        visited[i][j] = true;
        for (int k = 0; k < 4; k++) {
            int nrow = i + deltaRow[k];
            int nCol = j + deltaCol[k];
            if (nrow >= 0 && nrow < board.length && nCol >= 0 && nCol < board[0].length && board[nrow][nCol] == 1 && !visited[nrow][nCol]) {
